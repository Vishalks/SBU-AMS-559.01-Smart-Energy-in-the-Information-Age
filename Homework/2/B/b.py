...
Original file is located at
    https://colab.research.google.com/drive/1GmYu0UuroKxmERypUVNFyyy2RGlJuYZ0
"""
...
"""Thus as shown above, the static offline optimization gives the value of 0.902 kW and the objective function cost as 194.31. Since this is static optimization, we get a straight line as optimal on graph as above.
...
"""Thus as shown above, the static offline optimization different values of x for each time step and the objective function cost as 160.78 which is less than the static one. Since this is dynamic optimization, we get a boxed line as optimal on graph as above.
...
"""The graph above shows the variation of objective function value w.r.t the step size. Initially, the cost is higher but it falls drastically later and becomes the least at step size of 0.01789. After this, the objective function value again increases gradually."""
...
"""Thus as shown above, the online gradient descent optimization gives different values of x for each time step T and the least objective function cost 215.906 is obtained at step size 0.0179."""
...
path = "/content/drive/My Drive/AMS Smart Energy/data/prediction_results_B_extratrees.csv"
y_extratrees = pd.read_csv(path, usecols = ['prediction'])

"""# RHC and CHC with ExtraTrees and SVR for fixed value of a and b

SVR and ExtraTrees were found out to be the best algorithms for House B for prediction in Assignment 1. Following section describes use of RHC and CHC with both of them separately
"""
...
path = "/content/drive/My Drive/AMS Smart Energy/data/prediction_results_B_svr.csv"
...
for i in range(1,11):
    windows.append(i)
    costs.append(RHC(y_svr['prediction'].to_numpy(), i, 'SVR'))

fig = plt.figure(figsize=(24, 8))
plt.title("Prediction Window Values for RHC and SVR")
plt.plot(windows, costs, label = "Prediction Window Value")
plt.legend()
plt.ylabel("Cost Value")
plt.xlabel("Prediction Window Size")

"""The graph above shows the variation of objective function value for combination of RHC and SVR algorithm w.r.t the prediction window size. Initially, the cost is higher but it falls drastically later and becomes the least (239.01) at window size of 3. After this, the objective function value almost remains constant."""

x = RHC(y_svr['prediction'].to_numpy(), windows[costs.index(min(costs))], 'SVR')
print("Optimal cost for RHC and SVR found at window size: ", windows[costs.index(min(costs))])
print("\nOptimal cost is: ", x)

"""# CHC with ExtraTrees and SVR"""
...
"""The graph above shows the variation of objective function value for combination of CHC and ExtraTrees algorithm w.r.t the commitment horizon size. Initially, the cost is higher but it falls gradually later and becomes the least (188.25) at commitment horizon size of 10. After this, the objective function value almost remains constant."""
...
for i in range(1,11):
    windows.append(i)
    costs.append(CHC(y_svr['prediction'].to_numpy(), 10, i, 'SVR'))

fig = plt.figure(figsize=(24, 8))
plt.title("Commitment Horizon Values for CHC and SVR")
plt.plot(windows, costs, label = "Commitment Horizon Value")
plt.legend()
plt.ylabel("Cost Value")
plt.xlabel("Commitment Horizon Size")

"""The graph above shows the variation of objective function value for combination of CHC and SVR algorithm w.r.t the commitment horizon size. Initially, the cost is higher but it falls gradually later and becomes the least (168.78) at commitment horizon size of 9. After this, the objective function value almost remains constant."""

x = CHC(y_svr['prediction'].to_numpy(), 10, windows[costs.index(min(costs))], 'SVR')
print("Optimal cost for CHC and SVR found at commitment horizon size: ", windows[costs.index(min(costs))])
print("\nOptimal cost is: ", x)
...
"""Thus, some agorithms have a positive static regret while some have negative. CHC with SVR has the most negative static regret meaning it performs better than static optimal."""
...
"""Thus all agorithms have a positive dynamic regret meaning dynamic optimal performs better than all."""
...
plt.plot(decision_dict['RHC_SVR'], 'm', label="RHC with SVR")
plt.plot(decision_dict['CHC_ExtraTrees'], 'k', label="CHC with ExtraTrees")
plt.plot(decision_dict['CHC_SVR'], 'w', label="CHC with SVR")
plt.legend()
plt.ylabel("Electricity Units in kW")
plt.xlabel("Time step t(1 unit = 15 minutes)")
...
Best combination is CHC with SVR with commitment horizon size of 9 with a=4 and b=4. We keep commitment horizon size as constant and vary first a by keeping b constant and then vice-versa to see the impact of change.
"""
...
for i in range(1,11):
    windows.append(i)
    costs.append(chc_vary_and_b(y_svr['prediction'].to_numpy(), 10, 3, 'SVR', i, b))
...
"""The above graph shows the impact of changing values for a for combination of CHC and SVR with commitment horizon of 10. The cost goes on increasing with increase in a with some exceptions."""
...
for i in range(1,11):
    windows.append(i)
    costs.append(chc_vary_and_b(y_svr['prediction'].to_numpy(), 10, 3, 'SVR', a, i))
...
"""The above graph shows the impact of changing values for b for combination of CHC and SVR with commitment horizon of 10. The cost goes on increasing with increase in b.
...
"""As seen above, out of 672 windows, CHC wins the most number of times(446) and RHC the least(30). Thus, CHC is the winner.
